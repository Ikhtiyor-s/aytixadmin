        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {banners.map((banner) => (
            <div key={banner.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
              <div className="h-40 bg-gradient-to-r from-[#33ccff]/20 to-[#33ccff]/10 flex items-center justify-center">
                {banner.image_url ? (
                  <img
            ...
            <div className="p-6 space-y-4">
              ...
              {activeTab === 'banners' && (
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    {videoPreview && (
                      <video src={videoPreview} className="w-20 h-20 rounded-lg object-cover" autoPlay loop muted />
                    )}
                    {formData.video_url && !videoPreview && (
                      <video src={formData.video_url.startsWith('http') ? formData.video_url : `${API_URL}${formData.video_url}`} className="w-20 h-20 rounded-lg object-cover" autoPlay loop muted />
                    )}
                    <input
                      type="file"
